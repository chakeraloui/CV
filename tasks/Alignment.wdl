...
task SamToFastqAndBwaMemAndMba {
  ...
  command <<<
    ...
    if [ -s ~{reference_fasta.ref_alt} ]; then
      java -Xms1000m -Xmx1000m -jar /home/aloui/chaker/WD/tools/picard.jar  \
        SamToFastq \
        INPUT=~{input_bam} \
        FASTQ=/dev/stdout \
        INTERLEAVE=true \
      ...
      java -Dsamjdk.compression_level=~{compression_level} -Xms1000m -Xmx1000m -jar /home/aloui/chaker/WD/tools/picard.jar  \
        MergeBamAlignment \
...
task SamSplitter {
  ...
  command {
    ...
    java -Dsamjdk.compression_level=~{compression_level} -Xms3000m -Xmx3000m -jar /home/aloui/chaker/WD/tools/picard.jar  SplitSamByNumberOfReads \
      INPUT=~{input_bam} \
      OUTPUT=output_dir \
