...
task SortSam {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms4000m -Xmx5000m -jar /home/aloui/chaker/WD/tools/picard.jar \
      SortSam \
      INPUT=~{input_bam} \
...
task SortSamSpark {
  ...
  command {
    ...
    gatk --java-options "-Dsamjdk.compression_level=~{compression_level} -Xms4g -Xmx5g" \
      SortSamSpark \
      -I ~{input_bam} \
      -O ~{output_bam_basename}.bam \
      -- --conf spark.local.dir=. --spark-master 'local[6]' --conf 'spark.kryo.referenceTracking=false'
...
task MarkDuplicates {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms4g  -Xmx5g -jar /home/aloui/chaker/WD/tools/picard.jar \
      MarkDuplicates \
...
task MarkDuplicatesSpark {
  ...
  command <<<
    ...
    gatk --java-options "-Dsamjdk.compression_level=~{compression_level} -Xmx5g" \
      MarkDuplicatesSpark \
      --input ~{sep=' --input ' input_bams} \
      ...
      -- --conf spark.local.dir=/mnt/tmp --spark-master 'local[4]' --conf 'spark.kryo.referenceTracking=false'
  >>>
...
task BaseRecalibrator {
  ...
  command {
   gatk --java-options "-XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -XX:+PrintFlagsFinal \
      ...
      -Xloggc:gc_log.log -Xms1000m -Xmx2000m" \
      BaseRecalibrator \
      -R ~{ref_fasta} \
      -I ~{input_bam} \
...
task ApplyBQSR {
  ...
  command {
    gatk --java-options "-XX:+PrintFlagsFinal -XX:+PrintGCTimeStamps -XX:+PrintGCDateStamps \
      ...
      -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 -Dsamjdk.compression_level=~{compression_level} -Xms3000m -Xmx5000m" \
      ApplyBQSR \
...
task GatherBqsrReports {
  ...
  command {
    gatk --java-options "-Xms3000m -Xmx5000m" \
      GatherBQSRReports \
...
task GatherSortedBamFiles {
  ...
  command {
    java -Dsamjdk.compression_level=~{compression_level} -Xms2000m -Xmx5000m -jar /home/aloui/chaker/WD/tools/picard.jar \
      GatherBamFiles \
      INPUT=~{sep=' INPUT=' input_bams} \
      OUTPUT=~{output_bam_basename}.bam \
      CREATE_INDEX=true \
...
task GenerateSubsettedContaminationResources {
  ...
  command <<<
    ...
    grep -vE "^@" ~{target_interval_list} |
       awk -v OFS='\t' '$2=$2-1' |
       bedtools intersect -c -a ~{contamination_sites_bed} -b - |
...
task CheckContamination {
  ...
  command <<<
    ...
    /home/aloui/chaker/WD/tools/VerifyBamID.Linux/VerifyBamID \
    --Verbose \
    --NumPC 4 \
