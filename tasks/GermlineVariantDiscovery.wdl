...
task HaplotypeCaller_GATK35_GVCF {
  ...
  command {
    gatk --javaOptions "-Xms4g -Xmx4g" \
      ...
      -O local.sharded.bam \
    && \
    java -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10 --Xms4g -Xmx4g \
      -jar /home/aloui/chaker/WD/tools/gatk_GenomeAnalysisTK-3.5-0/gatk35 \
      -T HaplotypeCaller \
      -R ~{ref_fasta} \
...
task HaplotypeCaller_GATK4_VCF {
  ...
  command <<<
    set -e
    gatk --java-options "-Xms4g -Xmx4g  -XX:GCTimeLimit=50 -XX:GCHeapFreeLimit=10" \
      HaplotypeCaller \
      -R ~{ref_fasta} \
      -I ~{input_bam} \
      -L ~{interval_list} \
...
task MergeVCFs {
  ...
  command {
    java -Xms2g -Xmx6g-jar ../tools/picard.jar  \
      MergeVcfs \
...
task HardFilterVcf {
  ...
  command {
     gatk --java-options "-Xms3000m -Xmx4000m" \
      VariantFiltration \
      -V ~{input_vcf} \
      -L ~{interval_list} \
...
task CNNScoreVariants {
  ...
  command {
     gatk --java-options -Xms4g -Xmx4g CNNScoreVariants \
       -V ~{input_vcf} \
       -R ~{ref_fasta} \
       -O ~{output_vcf} \
       ~{bamout_param} \
...
task FilterVariantTranches {
  ...
  command {
    ...
    gatk --java-options -Xms4g -Xmx4g FilterVariantTranches \
      -V ~{input_vcf} \
